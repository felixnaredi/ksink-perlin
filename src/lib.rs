...
#[wasm_bindgen]
pub fn render(state: RenderStateDescriptor) -> Result<(), JsValue> {
    let context = state
        .canvas
        .get_context("2d")?
        .unwrap()
    ...
    context
        .scale(
            f64::from(state.canvas.client_width()) / f64::from(state.resolution_x),
            f64::from(state.canvas.client_height()) / f64::from(state.resolution_y),
        )
        .unwrap();
